import {getStore, createStore, replaceStore, reusable} from '../src/reusable';
...
describe('public store API', () => {
  ...
  it ('should allow to subscribe to creating a unit', () => {
    // TBD: not sure why this doesn't work: expect.assertions(1);
    const fn = () => {};
  ...
  it ('should allow to unsubscribe from subscribing to creating a unit', () => {
    // TBD: not sure why this doesn't work: expect.assertions(0);
    const fn = () => {};
...
xdescribe('reusable', () => {
  ...
  it ('should allow to use a reusable unit', () => {
    const useSomething = reusable(() => useState(0));

    expect(renderComp(useSomething)).toMatchSnapshot();
  });
});